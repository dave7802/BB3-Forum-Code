...
$report_spam	= (isset($_POST['report_spam'])) ? true : false;
...
if (in_array($mode, array('post', 'reply', 'quote', 'edit', 'delete', 'report_spam')) && !$forum_id)
{
...
switch ($mode)
{
	...
	case 'report_spam':
		...
		$sql = 'SELECT f.*, t.*, p.*, u.username, u.username_clean, u.user_sig, u.user_sig_bbcode_uid, u.user_sig_bbcode_bitfield, u.user_email
...
$post_data['post_user_email'] = (!empty($post_data['post_user_email'])) ? $post_data['post_user_email'] : ((!empty($post_data['user_email'])) ? $post_data['user_email'] : '');
...
// Handle report spam and delete post ...
if ($mode == 'report_spam')
{
...
if ($post_data['poster_id'] == ANONYMOUS)
{
	...
	$post_data['post_user_email'] = ($mode == 'quote' || $mode == 'edit') ? trim($post_data['post_user_email']) : '';
}
else
{
...
if ($submit || $preview || $refresh)
{
	...
	$post_data['post_user_email']	= utf8_normalize_nfc(request_var('user_email', $post_data['post_user_email'], true));
	...
	if ($config['enable_post_confirm'] && !$user->data['is_registered'] && in_array($mode, array('quote', 'post', 'reply')))
	{
		$captcha_data = array(
			...
			'user_email'	=> utf8_normalize_nfc(request_var('user_email', '', true)),
		);
	...
	/* Advanced Block MOD - renamed and moved to ./includes/abm/functions.php
	// DNSBL check
	...
	*/
	...
	if (!sizeof($error) && $submit)
	{
		...
		if ($submit)
		{
			...
			$redirect_url = submit_post($mode, $post_data['post_subject'], $post_data['username'], $post_data['topic_type'], $poll, $data, $update_message, ($update_message || $update_subject) ? true : false, $post_data['post_user_email']);
...
$template->assign_vars(array(
	...
	'USER_EMAIL'			=> ((!$preview && $mode != 'quote') || $preview) ? $post_data['post_user_email'] : '',
	...
	'S_DISPLAY_EMAIL'			=> ((!$user->data['is_registered'] || ($mode == 'edit' && $post_data['poster_id'] == ANONYMOUS)) && $config['require_email_guest']) ? true : false,
