...
// Recheck spam requested
if ($mode == 'recheck_spam' && $auth->acl_get('a_') && $session_id && $user->data['user_type'] != USER_IGNORE)
{
	include($phpbb_root_path . 'includes/functions_user.' . $phpEx);
...
while ($row = $db->sql_fetchrow($result))
{
	...
	$template->assign_block_vars('user_row', array(
		...
		'U_RECHECK_SPAM'	=> ($user->data['user_type'] != USER_IGNORE) ? append_sid("{$phpbb_root_path}viewonline.$phpEx", 'mode=recheck_spam&amp;s=' . $row['session_id']) : '',
