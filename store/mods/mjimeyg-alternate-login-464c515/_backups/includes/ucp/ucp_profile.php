		...
		switch ($mode)
		{
			case 'reg_details':
				...
				if ($submit)
				{
					...
					$error = array_merge($error, check_spam('user', 'profile', $user->ip, 0, 0, $data, false));
			...
			case 'profile_info':
				...
				include($phpbb_root_path . 'includes/functions_profile_fields.' . $phpEx);
                                ...
                                if( isset($tw_access_token->oauth_token) && $tw_access_token->oauth_token )
                                {   
				...
				if ($config['allow_birthdays'])
				{
					...
					$data['user_birthday'] = sprintf('%2d-%2d-%4d', $data['bday_day'], $data['bday_month'], $data['bday_year']);
                                ...
                                $data['al_tw_profile_sync'] = $tw_sync;
					...
					if (!sizeof($error))
					{
						...
						$sql_ary = array(
							...
							'user_interests'=> $data['interests'],
                                                        ...
                                                        'al_fb_profile_sync'    => $data['al_fb_profile_sync'],
                                                        'al_wl_profile_sync'    => $data['al_wl_profile_sync'],
                                                        'al_tw_profile_sync'    => $data['al_tw_profile_sync'],
				...
				$template->assign_vars(array(
					...
					'INTERESTS'	=> $data['interests'],
                                        'FB_WEBSITE'    => (isset($fb_user) ) ? $fb_user->website : '',
                                        'FB_LOCATION'   => (isset($fb_user) && isset($fb_user->location->name)) ? $fb_user->location->name : '',
                                        'FB_OCCUPATION' => (isset($fb_user) && isset($fb_user->work[0]->employer->name)) ? $fb_user->work[0]->employer->name : '',
                                    
                                        'WL_LOCATION'   => (isset($wl_user)) ? $user_locale : '',
                                        'WL_OCCUPATION' => (isset($wl_user) && isset($wl_user->work[0]->employer->name) ) ? $wl_user->work[0]->employer->name : '',
                                        
                                        'TW_WEBSITE'    => (isset($tw_user) && isset($tw_user->url) ) ? $tw_user->url : '',
                                        'TW_LOCATION'   => (isset($tw_user) && isset($tw_user->location) ) ? $tw_user->location : '',
			...
			case 'signature':
				...
				include($phpbb_root_path . 'includes/functions_display.' . $phpEx);
                                ...
                                if( isset($tw_access_token->oauth_token) && isset($user->data['al_tw_id']) )
                                {   
                                    include($phpbb_root_path . 'alternatelogin/twitter/tmhOAuth.' . $phpEx);
                                ...
                                $tw_status = (request_var('tw_status', '') == 'on') ? 1 : 0;
				...
				if ($submit || $preview)
				{
					...
					if (!sizeof($error))
					{
						...
						$error = array_merge($error, check_spam('user', 'profile', $user->ip, 0, 0, $data, $message_parser));
							...
							$sql_ary = array(
								...
								'user_sig_bbcode_uid'		=> (string) $message_parser->bbcode_uid,
                                                       'user_sig_bbcode_bitfield'	=> $message_parser->bbcode_bitfield,
                                                                ...
                                                                'al_tw_status_sync'             => ($tw_status == NULL) ? 0 : $tw_status,
							);
				...
				$template->assign_vars(array(
					...
					'S_BBCODE_FLASH'		=> ($config['allow_sig_flash']) ? true : false,
                             'S_LINKS_ALLOWED'		=> ($config['allow_sig_links']) ? true : false,
                                        ...
                                        'S_TW_STATUS'                   => isset($tw_user->status->text) ? $tw_user->status->text : '',
                                        )
				);
			...
			case 'avatar':
				...
				include($phpbb_root_path . 'includes/functions_display.' . $phpEx);
                     ...
                     include($phpbb_root_path . 'includes/functions_alternatelogin.' . $phpEx);
                                ...
                                $fb_url = 'https://graph.facebook.com/' . $user->data['al_fb_id'] . '/picture?type=normal';
				...
				if ($submit)
				{
					...
					{
                                                if($fb_avatar || $wl_avatar || $tw_avatar)
                                                {
				...
				$template->assign_vars(array(
					...
					'L_AVATAR_EXPLAIN'	=> sprintf($user->lang['AVATAR_EXPLAIN'], $config['avatar_max_width'], $config['avatar_max_height'], $config['avatar_filesize'] / 1024),
                                        'FB_AVATAR'             => $user->data['al_fb_avatar_sync'] ? 'checked="checked"' : '',
                                        'AL_FB_USER'            => $user->data['al_fb_id'],
                                        'TW_AVATAR'             => $user->data['al_tw_avatar_sync'] ? 'checked="checked"' : '',
                                        'AL_TW_USER'            => $user->data['al_tw_id'],
				));
