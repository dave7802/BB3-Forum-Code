...
class acp_inactive
{
	...
	function main($id, $mode)
	{
		...
		{
			if ($action !== 'delete' && $action !== 'report_spammer' && !check_form_key($form_key))
			{
			...
			switch ($action)
			{
				...
				case 'report_spammer':
					...
					else if ($action == 'delete' || $action == 'report_spammer')
					{
						if (confirm_box(true))
						{
							...
							{
								user_delete((($action == 'report_spammer') ? 'remove' : 'retain'), $user_id, $user_affected[$user_id]);

								if ($action == 'report_spammer')
								{
		...
		$option_ary = array('activate' => 'ACTIVATE', 'delete' => 'DELETE', 'report_spammer' => 'USER_ADMIN_REPORT_SPAMMER');
