...
class ucp_alternatelogin
{
	...
	function main($id, $mode)
	{
		...
		if($user->data['al_fb_id'])
		{
			...
			$fb_website = (!isset($fb_user->website)) ? false : $fb_user->website;
            $fb_location = (!isset($fb_user->location->name)) ? false : $fb_user->location->name;
            $fb_occupation = (!isset($fb_user->work[0]->employer->name)) ? false : $fb_user->work[0]->employer->name;
		...
		$template->assign_vars(array(
			...
			'S_FB_WEBSITE'									=> (isset($fb_website)) ? $fb_website : '',
			'S_FB_LOCATION'									=> (isset($fb_location)) ? $fb_location : '',
			'S_FB_OCCUPATION'								=> (isset($fb_occupation)) ? $fb_occupation : '',
			'S_FB_BIRTHDAY'									=> (isset($fb_birthday)) ? $fb_birthday : '',
			
			'S_FB_AVATAR'									=> (isset($fb_avatar->url)) ? $fb_avatar->url : '',
			
			'S_FB_STATUS'									=> (isset($fb_status)) ? $fb_status : '',
			...
			'S_WL_BIRTHDAY'									=> (isset($wl_birth_day) && isset($wl_birth_month) && isset($wl_birth_year)) ? $wl_birth_day . '-' . $wl_birth_month . '-' . $wl_birth_year : '',
