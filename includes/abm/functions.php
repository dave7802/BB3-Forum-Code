...
* @version $Id: functions.php, v 1.000 2012/05/05 Martin Truckenbrodt Exp$
		...
		if ($ip)
	{
		...
		while (($row = $db->sql_fetchrow($result)) && ($weight < 5 || $action == 'recheck'))
		{
			if (phpbb_checkdnsrr($reverse_ip . '.' . $row['iprbl_fqdn'] . '.', 'A') === true)
			{
				...
				elseif ($weight > 4 && $action == 'recheck')
				{
...
function check_httpbl($action = 'recheck', $ip = false, $username = false, $email = false, $message = false)
{
	...
	if (($ip || $username || $email || $message))
	{
		...
		while ($row = $db->sql_fetchrow($result))
		{
			...
			if ($row['httpbl_name'] == 'sfs')
			{
				...
				if ($file !== false)
				{
					...
					foreach ($file as $line)
					{
						...
						elseif (strpos($line, '<appears>') !== false && strpos($line, '</appears>') !== false)
						{
							$start = strpos($line, '<appears>') + 9;
							$end = strpos($line, '</appears>') - $start;
							$appears = (substr($line, $start, $end) == 'yes') ? true : false;
						}
						elseif (strpos($line, '<frequency>') !== false && strpos($line, '</frequency>') !== false)
						{
			...
			elseif ($row['httpbl_name'] == 'botscout')
			{
			...
			elseif ($row['httpbl_name'] == 'akismet' && $row['httpbl_key'])
			{
			...
			// a key always is needed
			elseif ($row['httpbl_name'] == 'honeypot' && $row['httpbl_key'] && $ip)
			{
				$apiurl = '';
				...
				if ($response != $query)
				{
					...
					if ($response[1] > 0 && $response[2] > 0 && $response[3] > 1 && count($response) == 4)
					{
						$types_found = 'ip';
						$block_spam = true;
					}
					elseif ($response[0] != '127')
					{
			...
			elseif ($row['httpbl_name'] == 'bde' && ($action == 'register' || $action == 'profile') && $email)
			{
				$apiurl = '';
...
function check_spam($mode = false, $action = 'recheck', $ip = false, $user_id = 0, $post_id = 0, $data = false, $message = false)
{
	...
	// IPRBL check
	if ($user_data['user_ip'] && ((($config['check_iprbl_post'] == CHECK_ALL || (($config['check_iprbl_post'] == CHECK_GUESTS) && $user_data['user_id'] == ANONYMOUS) && $mode == 'post') || ($config['check_iprbl_register'] && $mode == 'user')) || $action == 'recheck'))
	{
		if (($iprbl_array = check_iprbl($action, $user_data['user_ip'])) && array_key_exists('blacklists', $iprbl_array))
		{
			if ($action == 'recheck')
			{
				if (($config['check_iprbl_post'] == CHECK_ALL || (($config['check_iprbl_post'] == CHECK_GUESTS) && $user_data['user_id'] == ANONYMOUS) && $mode == 'post') || ($config['check_iprbl_register'] && $mode == 'user'))
				{
	...
	// HTTPBL check
	if (($user_data['user_ip'] || $user_data['username'] || $user_data['user_email'] || $message) && (((($config['check_httpbl_register'] && $mode == 'user') || ((($config['check_httpbl_post'] == CHECK_ALL) || ($config['check_httpbl_post'] == CHECK_GUESTS && $user_data['user_id'] == ANONYMOUS)) && $mode == 'post')) && ($config['break_after_iprbl'] && $info) == false) || $action == 'recheck'))
	{
		if (($httpbl_array = check_httpbl($action, $user_data['user_ip'], $user_data['username'], $user_data['user_email'], $message)) && array_key_exists('blacklists', $httpbl_array))
		{
			if ($action == 'recheck')
			{
				if (($config['check_httpbl_register'] && $mode == 'user') || (($config['check_httpbl_post'] == CHECK_ALL || (($config['check_httpbl_post'] == CHECK_GUESTS) && $user_data['user_id'] == ANONYMOUS) && $mode == 'post')))
				{
	...
	if ($user_data['user_email'] && ((($config['check_domainrbl_email'] == CHECK_ALL || $config['check_domainrbl_email'] == CHECK_GUESTS) && ($config['break_after_httpbl'] && $info) == false) || $action == 'recheck'))
	{
		...
		if (($email_domainrbl_array = check_domainrbl($action . '_email', $email_uris_array)) && array_key_exists('blacklists', $email_domainrbl_array))
		{
			if ($action == 'recheck')
			{
				if ($config['check_domainrbl_email'] == CHECK_ALL || $config['check_domainrbl_email'] == CHECK_GUESTS)
				{
	...
	if ($mode == 'post')
	{
		...
		if (isset($uris_array) && $uris_array && (((($config['check_domainrbl_post'] == CHECK_ALL) || ($config['check_domainrbl_post'] == CHECK_GUESTS && $user_data['user_id'] == ANONYMOUS)) && ($config['break_after_httpbl'] && $info) == false) || $action == 'recheck'))
		{
			if (($domainrbl_array = check_domainrbl($action, $uris_array)) && array_key_exists('blacklists', $domainrbl_array))
			{
				if ($action == 'recheck')
				{
					if ($config['check_domainrbl_post'] == CHECK_ALL || (($config['check_domainrbl_post'] == CHECK_GUESTS)))
					{
						if (array_key_exists('blocked', $domainrbl_array))
						{
...
// for future use if other databases will need it
	...
	while ($row = $db->sql_fetchrow($result))
	{
		if (($row['httpbl_name'] == 'sfs') && $row['httpbl_key'] && $ip && $username && $email && $row['httpbl_use_for_report'] && $row['httpbl_active_for_report'])
		{
			$apiurl = 'username=';
			if ($username && $row['httpbl_use_username'] && $row['httpbl_check_username'])
			{
...
function get_base_domain($full_domain = '', $reverse_ip = true)
{
	...
	elseif D0 = ctld && D1 = gtld && D2 == 'www', domain = D1.D0
	else domain = D1.D0
